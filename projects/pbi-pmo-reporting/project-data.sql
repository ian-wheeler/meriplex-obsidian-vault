SELECT 
	...
	CAST (
		CASE
			WHEN 
				CASE
					WHEN DATEDIFF(DAY, p.Date_Start, p.Date_Req) = 0 THEN 0 
					ELSE CAST(DATEDIFF(DAY, p.Date_Start, te_all.End_Date) AS NUMERIC(11, 2)) 
						 / CAST(DATEDIFF(DAY, p.Date_Start, p.Date_Req) AS NUMERIC(11, 2))
				END > 1 THEN 1 
			ELSE 
				CASE
					WHEN DATEDIFF(DAY, p.Date_Start, p.Date_Req) = 0 THEN 0 
	...
	----------------------------------------------------------------------------------------------------------------------------------
	-- Calculate 'Scheduled Percent Complete' based on whether the budget is estimated or summed from a budget summary.
	-- The calculation varies if the budget flag is set to estimated hours (1) or hours budget sum (0).
	-- It uses a CASE statement to handle division by zero and ensure numeric precision in the result.
	CAST(
		CASE
			-- Determine the denominator based on the Budget_Flag
			WHEN CASE 
					WHEN p.Budget_Flag = 1 THEN p.Est_Hours       -- If Budget_Flag is 1, use Est_Hours
					WHEN p.Budget_Flag = 0 THEN sr.Hours_Budget_Sum -- If Budget_Flag is 0, use Hours_Budget_Sum
				 END = 0 THEN 0  -- If the determined denominator is 0, result is 0 to avoid division by zero
			ELSE 
				sch.Scheduled_Hours / NULLIF( -- Else, divide Scheduled_Hours by the determined denominator, handling division by zero
					CASE 
						WHEN p.Budget_Flag = 1 THEN p.Est_Hours
						WHEN p.Budget_Flag = 0 THEN sr.Hours_Budget_Sum
					END, 0)
		END AS NUMERIC(9, 2)
	) AS 'Scheduled Percent Complete',
	----------------------------------------------------------------------------------------------------------------------------------
	...
	----------------------------------------------------------------------------------------------------------------------------------
	-- Calculate 'Actual Percent Complete' based on whether the budget is estimated or summed from a budget summary.
	-- This calculation compares actual hours worked to either estimated hours or a summed hours budget,
	-- depending on the Budget_Flag, ensuring division by zero is handled.
	CAST(
		CASE
			-- Determine the denominator based on the Budget_Flag
			WHEN CASE 
					WHEN p.Budget_Flag = 1 THEN p.Est_Hours       -- If Budget_Flag is 1, use Est_Hours
					WHEN p.Budget_Flag = 0 THEN sr.Hours_Budget_Sum -- If Budget_Flag is 0, use Hours_Budget_Sum
				 END = 0 THEN 0  -- If the determined denominator is 0, return 0 to avoid division by zero
			ELSE 
				te_all.Hours_Actual_SUM / NULLIF( -- Else, divide actual hours by the determined denominator, handling division by zero
					CASE 
						WHEN p.Budget_Flag = 1 THEN p.Est_Hours
						WHEN p.Budget_Flag = 0 THEN sr.Hours_Budget_Sum
					END, 0)
		END AS NUMERIC(9, 2)
	) AS 'Actual Percent Complete',
	----------------------------------------------------------------------------------------------------------------------------------
...
WHERE DATEADD (Month, -36 , CURRENT_TIMESTAMP) <= p.Date_Start